...
//#include "ttH-LeptonPlusJets/AnalysisCode/interface/newtest.h"
...
void Yggdrasil_TreeReader_2017(  int SampleType=2,int SplitType =2, int maxNentries=-1, int Njobs=1, int jobN=1) {

gSystem->Load("libttH-LeptonPlusJetsAnalysisCode");
gSystem->Load("libttH-LeptonPlusJetsYggdrasilTreeMaker");
...
int useCondor = 1; //Important for i/o file extensions
  ...
  if(SampleType==3)treefilename="/store/user/sflowers/44949CF4-96C6-E611-B9A0-0025905A6122.root";
  
  //old files: plz ignore!!
  /*
  ...
  */
  ...
  if(SampleType == 999)treefilename = "$CMSSW_BASE/src/ttH-LeptonPlusJets/YggdrasilTreeMaker/test/yggdrasil_treeMaker2017.root";
  if(SampleType == 999)sampleName = "test";
  ...
  if(SampleType ==2)std::cout<< "  +  "<<treefilename2.c_str()<<std::endl;
  if(SampleType ==2)std::cout<< "  +  "<<treefilename3.c_str()<<std::endl;
  ...
  if(SampleType ==2)chain->Add(treefilename3.c_str());   // for longer samples
  if(SampleType ==2)chain->Add(treefilename2.c_str());
...
/////Event Selection
		...
		bool pass_METFilters = eve->pass_METFilters_;
		bool pass_MuFilters = eve->pass_MuFilters_;
		
		int ElTrig = eve->passHLT_Ele27_eta2p1_WPTight_Gsf_v_;
		int MuTrig = (eve->passHLT_IsoMu24_v_ ==1 || eve->passHLT_IsoTkMu24_v_==1) ? 1 : 0;
		
		std::vector<double> lepton_pt = eve->lepton_pt_;  
    		std::vector<double> lepton_eta = eve->lepton_eta_;
    		std::vector<double> lepton_scEta = eve->lepton_scEta_;
    		std::vector<double> lepton_phi = eve->lepton_phi_;
    		std::vector<double> lepton_e = eve->lepton_e_;
    		std::vector<int>    lepton_trkCharge = eve->lepton_charge_;
    		std::vector<int>    lepton_isMuon = eve->lepton_isMuon_;
    		std::vector<double> lepton_relIso = eve->lepton_relIso_;
    		std::vector<int>    lepton_isTight = eve->lepton_isTight_;
    		std::vector<int>    lepton_isLoose = eve->lepton_isLoose_;
		
		Float_t MET_pt = eve->MET_[0];
		Float_t MET_phi = eve->MET_phi_[0];
		
		TLorentzVector TLMET;
	  	TLMET.SetPtEtaPhiE(eve->MET_[0],0,eve->MET_phi_[0],eve->MET_[0]);
		...
		// JET SELECTION
				...
				std::vector<double> jet_pt;
     				...
     				std::vector<int> jet_flavour;
		                ...
		                if(iSys>0 || iSys<5){
		 	   		jet_pt = eve->jet_pt_[iSys];
     					jet_eta = eve->jet_eta_[iSys];
     					jet_phi = eve->jet_phi_[iSys];
     					jet_m = eve->jet_m_[iSys];
     					jet_csvV = eve->jet_combinedInclusiveSecondaryVertexV2BJetTags_[iSys];
     					jet_flavour = eve->jet_flavour_[iSys];
				}
				else{
					jet_pt = eve->jet_pt_[0];
     					jet_eta = eve->jet_eta_[0];
     					jet_phi = eve->jet_phi_[0];
     					jet_m = eve->jet_m_[0];
     					jet_csvV = eve->jet_combinedInclusiveSecondaryVertexV2BJetTags_[0];
     					jet_flavour = eve->jet_flavour_[0];
				}
    ...
    selection . SetElElTrigger(  ( 0  ) );
    selection . SetElMuTrigger(  0 );
    selection . SetMuMuTrigger(  0 );
    ...
    selection . SetMuMETFilters( & (pass_MuFilters), & (pass_METFilters));

  
  
  selection.doEventSelection();
  bool selected = true;
  if(!(selection . PassSingleElCh()) && !(selection.PassSingleMuCh()))selected=false;
  if(!selected)continue;
				
				
		        int numJets=selection.jets().size();
			int numTags=selection.bjets().size();
		  ...
		  // Get CSV Weights		
				...
				std::vector<double> ptcsv, etacsv  ; 
  for( unsigned int i = 0 ; i < selection.jets().size() ; i++ ){
    ptcsv  .push_back( selection.jets().at(i) -> Pt()  );
    etacsv .push_back( selection.jets().at(i) -> Eta() );
  }
				
				double newCSVwgt = 1;
				double dummy = -1;
				//newCSVwgt = scalefactors.get_csv_wgt(ptcsv,etacsv,selection.jetsBdiscriminant(),selection.jetsFlav(),iSys+6,dummy,dummy,dummy);
				//newCSVwgt = scalefactors.get_csv_wgt( & selection, iSys+6, dummy, dummy, dummy);
				double wgt=MyWgt;
				double nocsvwgt=wgt;
				wgt=wgt*newCSVwgt;
